...
Reads leads from leads.csv, sends each lead's Name, Business_Type, Budget,
and Message to Gemini for evaluation, drafts a personalized follow-up email
for High-priority leads, and writes the results to qualified_leads.json.
"""
...
For each lead, assess the Business Type, Budget, and Message to assign a priority:
...
Write exactly one sentence for the justification, referencing the specific budget and need.

When priority is High, also draft a highly professional, personalized follow-up sales email
(with a subject line) that references the lead's specific business type, stated need, and
budget, and proposes a clear next step. For Medium/Low leads, leave follow_up_email as null."""
...
def evaluate_lead(client: genai.Client, lead: pd.Series) -> LeadEvaluation:
...
def main() -> None:
    ...
    print(f"Loaded {total} leads from {INPUT_CSV}\n")
    ...
    for i, (_, lead) in enumerate(leads_df.iterrows()):
        ...
        print(f"[{i + 1}/{total}] Analyzing lead: {lead['Name']} ({lead['Business_Type']})...")
        try:
            evaluation = evaluate_lead(client, lead)
            record["priority"] = evaluation.priority
            record["justification"] = evaluation.justification
            record["follow_up_email"] = evaluation.follow_up_email
            record["status"] = "success"

            email_note = "follow-up email drafted" if evaluation.follow_up_email else "no email needed"
            print(f"    -> Priority: {evaluation.priority} - {email_note}")
    ...
    print(f"\nDone. {succeeded}/{total} leads analyzed. Results saved to {OUTPUT_JSON}")
